...
void handler_result(int result)
{
    ...
    LOGLOG("Filterui: result:%d" ,result);
...
//    setWindowFlags(Qt::FramelessWindowHint);
    ...
    LOGLOG("Filterui: jobid:%d" ,jobid);
    m_userManage.initdll();
    LOGLOG("mainwindow init dll ok");
}
...
//        if(m_userManage.isValidUser("aa","123","AXY-HS1610"))
        if(m_userManage.isValidUser(ui->lineEdit->text().toUtf8().data(),
                                    ui->lineEdit_2->text().toUtf8().data(),
                                    printer_name.toUtf8().data()))
        {
            result = Filterlib_Status_OK;
        }
    }
