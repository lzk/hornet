...
//      else if((pStatus->PaperTrayStatus & 0x01) && !(pStatus->PaperTrayStatus & 0x02) && (pStatus->PaperTrayStatus & 0x04))
//          ps->PaperTray = 5;
    ps->PaperTrayStatus = pStatus->PaperTrayStatus;
...
int StatusMonitor::getDeviceStatus(DeviceIO* device ,PrinterInfo_struct* printer)
{
    return getDeviceStatus(device ,&printer->printer ,&printer->status);
}
