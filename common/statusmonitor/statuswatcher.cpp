...
int getDevices(const char* url,const char* deviceid ,void* para)
{
    WatcherStatusThread* wst = (WatcherStatusThread*) para;
    if(wst){
        wst->urls << url;
        wst->deviceids << deviceid;
    }
    return 0;
}
#if 0
...
//    LOGLOG("printer %s get device status" ,printerinfo->printer.name);
    urls.clear();
    deviceids.clear();
    result = cups_getDevices("usb,socket" ,10 ,getDevices ,(void*)this);
    QString deviceid;
    for(int i = 0 ;i < urls.count() ;i++) {
        if(!current_url.compare(urls[i])){
    ...
    if(deviceid.isEmpty()){
        ...
        result = DecodeStatusFromDeviceID(deviceid.toLatin1().data() ,&status);
...
void StatusWatcher::set_current_printer(const QString& printer)
{
    ...
    memset(&current_printer_info.status ,-1 ,sizeof(PrinterStatus_struct));
    if(!printer.isEmpty())
        strcpy(current_printer_info.printer.name ,printer.toLatin1().constData());
    mutex.unlock();
    ...
    if(printer.isEmpty()){
        ...
        if(statusThread){
            ...
            if(statusThread->device && statusThread->device->is_the_same_device(&printerinfo.printer)){
                ...
                statusThread->current_url = printerinfo.printer.deviceUri;
                mutex.unlock();
...
#if 1
    current_printer_info.printer = pis->printer;
    if(!pis->printer.status){//valid status
//        current_printer_info = *pis;
        current_printer_info.status = pis->status;
    }
    mutex.unlock();
    update_current_printer_status();
    return;
